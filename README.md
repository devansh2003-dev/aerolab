...
**Second 3D regime — sphere Re = 20 (2026-06-01).** Steady-wake companion to the Re = 100 sphere case. Same body, grid, and blockage; only ν and U scaled. Cd = **4.265** vs CGW 1978 reference 2.73 (**+56.2 %**). The Ladd-1994 + D = 20 bias is slightly Re-dependent, growing with the viscous fraction of drag — physically consistent with the §8.3.3 / §8.3.4 diagnosis. This Re = 20 data point still uses the **old** Ladd + D = 20 configuration and **remains preview-quality**; a method-consistent re-run at D = 40 + MYSL is queued (see [VALIDATION.md §8.8](VALIDATION.md)). See [VALIDATION.md §8.3.2](VALIDATION.md).
...
**MYSL 2002 q-aware momentum exchange closes the bulk of the 3D gap at Re = 100 (2026-06-01, v1.7.0).** Implemented the Mei-Yu-Shyy-Luo 2002 Bouzidi-aware momentum-exchange formula (Phys. Rev. E 65, 041203) and re-evaluated the SAME D = 40 converged flow with the new force formula. Result: **Sphere Re = 100 drag with MYSL + D = 40 lands at Cd = 1.160 / +6.44 % vs Clift-Grace-Weber 1978** — a 33.8 percentage-point bias reduction vs the Ladd post-stream form on the same flow. **This is a single-configuration result** (one Reynolds number, one body, one grid, one blockage) and not a general 3D-drag validation; the Re = 20 companion is still at the Ladd + D = 20 baseline (+56 %), and a method-consistent re-run is queued. See [VALIDATION.md §8.3.4](VALIDATION.md).
...
│   ├── forces.py                   # Ladd 1994 momentum exchange (2D)
│   ├── forces_3d.py                # Ladd 1994 + MYSL 2002 q-aware momentum exchange (3D)
