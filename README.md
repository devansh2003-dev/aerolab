...
**AeroLab ↔ OpenFOAM Strouhal cross-check (2026-06-01).** Long-record (158 D/U) cylinder bake at the Validation preset, FFT of the lift coefficient on 28 saturated shedding cycles. AeroLab St = **0.1794** vs Williamson 0.166 (**+8.07 %**) vs OpenFOAM 0.1600 (**+12.13 %**). AeroLab raw and OpenFOAM raw bracket Williamson from opposite sides; the +8 % AeroLab gap is honest — raw Cd at this preset is also biased high (+14 %), and the Allen-Vincenti correction acts on Cd only. See [VALIDATION.md §8.4](VALIDATION.md) and [`data/validation/cylinder_re100_strouhal_lowblockage.json`](data/validation/cylinder_re100_strouhal_lowblockage.json).

**Second 3D regime — sphere Re = 20 (2026-06-01).** Steady-wake companion to the Re = 100 sphere case. Same body, grid, and blockage; only ν and U scaled. Cd = **4.265** vs CGW 1978 reference 2.73 (**+56.2 %**). For context, Re = 100 gives Cd = 1.645 / +51 %; the Ladd-1994 + D = 20 bias is slightly Re-dependent, growing with the viscous fraction of drag. 3D drag is now anchored on **two** Re-points, both still preview-quality. See [VALIDATION.md §8.3.2](VALIDATION.md).

**D = 40 sphere bake falsifies the grid-resolution hypothesis (2026-06-01).** Re-ran the sphere Re = 100 case at **8 × the cell count** (320 × 160 × 160, D = 40 instead of D = 20), holding the simplified Ladd 1994 momentum exchange constant. Result: Cd = **1.528 / +40.2 %** vs CGW 1.09 — the D-refinement closed only **~ 7 percentage points** of the +51 % D = 20 baseline bias. The remaining ~ 33 % lives in the simplified Ladd momentum exchange, not the grid. See [VALIDATION.md §8.3.3](VALIDATION.md).

**MYSL 2002 q-aware momentum exchange closes the bulk of the 3D gap (2026-06-01, v1.7.0).** Implemented the Mei-Yu-Shyy-Luo 2002 Bouzidi-aware momentum-exchange formula (Phys. Rev. E 65, 041203) and re-evaluated the SAME D = 40 converged flow with the new force formula. Result: Cd = **1.160 / +6.44 %** vs CGW 1.090 — a **33.8 percentage-point** bias reduction vs the Ladd post-stream form on the same flow. **3D bluff-body drag is no longer preview-quality** — it is validated against an experimental reference at near-percent level. See [VALIDATION.md §8.3.4](VALIDATION.md).
...
- **Live 3D solve on Cloud.** The 3D D3Q19 TRT kernel runs offline (~20 s per scene on a laptop); the hosted app replays the saved velocity field. 3D Re tops out at 100 in the shipped bakes (Re=200 BGK/TRT diverged at our grid resolution — tau ≈ 0.512). **Near-percent-level 3D drag validation as of v1.7.0** — D = 40 sphere + MYSL 2002 q-aware momentum exchange lands at Cd = 1.160 / **+6.44 %** above Clift-Grace-Weber 1978. The remaining ~ 6 % breaks down into known refinement sources (residual blockage at B = 25 %, D = 40 voxelisation versus Mei-Luo-Shyy 1999's D ≥ 60, Bouzidi quadratic BB); a future D = 60 / B = 10 % bake should push under 2 % (see [VALIDATION.md §8.3.4](VALIDATION.md)).
